...
1. После изменения кода — прогнать тесты и compileall.
   При изменении веб-интерфейса (шаблоны, роуты, поведение страниц) — обязательно актуализировать e2e-тесты в `tests/e2e/` (добавить для новой функциональности, изменить затронутые, удалить для убранной) и прогнать `pytest -m e2e`.
