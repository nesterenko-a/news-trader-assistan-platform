...
Полная спецификация проекта — в `docs/` (начните с `docs/README.md`; комплект из 19 документов + 20, текущая версия v1.79). Правила работы — в `docs/16-working-process.md` — **обязательны к выполнению**.
...
- `tests/` — pytest (SQLite)
- `docs/` — документация (версионируется, v1.77)
...
- Документация: комплект **v1.77** (инициализирован 2026-08; версии: 07 — v1.15, 12 — v1.30, 13 — v1.30, 14 — v1.38, 19 — v1.12, 20 — v1.0, остальные см. шапки).
- Roadmap (12): этап 0 завершён; этап 1 — срезы 1–6 + News Sources Manager (страница `/news`, ленты из БД); этап 2 — начат (paper trading, контраргументы/риски, веса факторов, OI, Volume Profile, EMA, MACD).
