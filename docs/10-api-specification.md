...
**Статус:** утверждено v1.32 (добавлено управление остановкой фоновых задач)
...
#### Кандидаты связей knowledge graph (администратор)

Кандидаты создаются из проанализированных новостей и не влияют на стратегии до принятия. Все маршруты требуют роль `admin`.

- `GET /v1/admin/graph/candidates` — очередь (`status`, `entity`, `direction`, `min_confidence`, `page`, `per_page`); ответ содержит кандидатов и их evidence.
- `GET /v1/admin/graph/candidates/{id}` — кандидат и статьи-доказательства.
- `POST /v1/admin/graph/candidates/{id}/approve` — принять; допускает корректировку `direction`, `strength`, `kind`, `confidence`, `rationale`, `source_ref`, `comment`; создаёт `Influence`.
- `POST /v1/admin/graph/candidates/{id}/reject` — отклонить с необязательным `{comment}`.
- `POST /v1/admin/graph/candidates/{id}/need-evidence` — отметить как ожидающий дополнительных независимых доказательств.

`409` возвращается, если кандидат уже обработан или утверждённое ребро этой пары уже существует.
При повторных evidence агрегированная `strength` определяется большинством предложений; равенство разрешается в пользу более осторожного значения.
...
- Админ-настройки (роут страницы, только роль `admin`):
  ...
  - `POST /admin/realtime/toggle` — включить или выключить realtime. При включении создаёт запуск `realtime_updater`; при выключении отключает конфигурацию и завершает живой процесс. Принимает локальный `return_to` (`/admin` или `/settings/admin`).
  - `POST /admin/scripts/{run_id}/stop` — остановить конкретный живой запуск; возвращает 409, если процесс уже завершён или не принадлежит текущему приложению. Для realtime дополнительно выключает конфигурацию.
  - `GET /settings/admin` — компактная панель управления: состояние realtime, доступные задачи, активные notices и последние запуски.
