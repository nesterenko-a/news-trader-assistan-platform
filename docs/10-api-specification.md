...
- `PUT /alerts/settings` — изменить `{ "min_impact"?, "channels"? }` (каналы: app / telegram; telegram — этап 1, планируется).

Доставка в интерфейсе — мгновенная (список на странице); push-каналы — этап 1, позже.
