...
- Админ-настройки (роут страницы, только роль `admin`):
  ...
  - `POST /admin/realtime/toggle` — включить или выключить realtime. При включении создаёт запуск `realtime_updater`; при выключении отключает конфигурацию и завершает живой процесс. Принимает безопасный локальный `return_to`.
