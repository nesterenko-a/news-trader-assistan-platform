...
### 2.4.1. GraphCandidate и GraphCandidateEvidence (кандидат связи)

Автоматически извлечённая связь не становится частью рабочего графа до решения куратора. `GraphCandidate` хранит пару известных сущностей, предполагаемые знак/силу/характер связи, агрегированную уверенность, статус (`needs_evidence` / `pending` / `approved` / `rejected`) и аудит решения. После принятия поле `approved_influence_id` ссылается на созданный обычный `Influence`.

`GraphCandidateEvidence` связывает кандидата со статьёй, содержит цитату, объяснение, уверенность и предложенную силу отдельного извлечения. Одна статья не дублируется для одного кандидата. Активный кандидат уникален для пары, направления и характера связи; повторные статьи добавляются как evidence. Итоговая сила выбирается по большинству доказательств, а при равенстве — в пользу более осторожного значения. Кандидаты и их доказательства не используются движком стратегий, пока не создано утверждённое ребро.
...
Элементы `research` создаются движком для цепочек влияния, ребро которых ссылается на научную/аналитическую работу (`Influence.source_ref`); вес 0, по уникальной ссылке — одна запись (FR-05-08, см. [09-strategy-engine.md](./09-strategy-engine.md) §3).
...
| status | enum | running / success / failed / stopped |
...
| stopped_by | FK, null | Администратор, остановивший живой процесс |
| stopped_at | datetime, null | Время штатной или принудительной остановки |
...
Персональная настройка администратора для конвейера (по образцу `alert_settings`). Сейчас хранит последний выбранный шаблон фьючерсов фазы 2.

| Поле | Тип | Описание |
|---|---|---|
| user_id | PK, FK → users | Пользователь |
| last_futures_template_id | FK → futures_templates | Шаблон фьючерсов, выбранный по умолчанию (null — не выбран) |

Записывается через `POST /admin/pipeline/set-template`; применяется при запуске Ежедневного конвейера с фазами 1–2 (в запуск подставляется `--tickers` из шаблона).
...
### 2.14.1. UserProfile (профиль пользователя)

Одна необязательная приватная запись на пользователя. Профиль отделён от учётной записи `User`, чтобы личные данные не попадали в рабочие сущности и ответы аналитики.

| Поле | Тип | Описание |
|---|---|---|
| user_id | PK, FK | Пользователь-владелец |
| full_name | string, nullable | ФИО |
| display_name | string, nullable | Отображаемое имя |
| avatar_path | string, nullable | Внутренний путь к обработанному аватару WebP |
| created_at, updated_at | datetime | Время создания и изменения |
