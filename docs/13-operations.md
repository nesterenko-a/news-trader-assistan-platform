...
| `scripts/collect_news.py` | Собирает RSS, фильтрует релевантное, анализирует через LLM, сохраняет в БД (шаг конвейера). Окно сбора: `--days N` / `--from YYYY-MM-DD`; точечный сбор по сущностям: `--entity <имя>` (повторяемый); Telegram-каналы: `--telegram`. **Сайты компаний (`--sites`) — в планах, ТЗ: [22-company-sites-source.md](./22-company-sites-source.md)** |
...
**E2E-тесты веб-интерфейса (Playwright):** сценарии в `tests/e2e/` (маркер `e2e`) запускаются **отдельным прогоном** — `pytest -m e2e` (браузеры Playwright должны быть установлены); в общий `pytest -q` не входят (sync Playwright конфликтует с pytest-asyncio, см. [21-web-e2e-tests.md](./21-web-e2e-tests.md)). Быстрый запуск из корня: `scripts\run_e2e.bat` (аргументы пробрасываются, например `scripts\run_e2e.bat tests/e2e/test_public.py`).
