...
| `scripts/collect_news.py` | Собирает RSS, фильтрует релевантное, анализирует через LLM, сохраняет в БД (шаг конвейера). Окно сбора: `--days N` (последние N дней) или `--from YYYY-MM-DD` (с даты) |
