...
**Запуск скриптов из веб-интерфейса:** администраторы (роль `admin`, см. [11-security-compliance.md](./11-security-compliance.md)) могут запускать основные скрипты через страницу `/admin` — с историей запусков и **живым логом** (вывод поступает в реальном времени, скрипты запускаются без буферизации и сообщают о ходе работы). Журнал хранится в таблице `script_runs`. При перезапуске сервиса запуски, оставшиеся в статусе «выполняется», автоматически помечаются как прерванные (failed).
