...
**E2E-тесты веб-интерфейса (Playwright):** сценарии в `tests/e2e/` (маркер `e2e`) запускаются **отдельным прогоном** — `pytest -m e2e` (браузеры Playwright должны быть установлены); в общий `pytest -q` не входят (sync Playwright конфликтует с pytest-asyncio, см. [21-web-e2e-tests.md](./21-web-e2e-tests.md)).
