...
## 3. Планировщик сбора новостей

Сбор новостей выполняется автоматически по расписанию — эквивалент cron на Windows (Task Scheduler).

- Задача: `NewsTraderBot\CollectNews`.
- Расписание: ежедневно в 09:00.
- Скрипт: `scripts/collect_news.bat` (активирует venv и запускает `scripts/collect_news.py`).
- Лог: `logs/collect_news.log`.
...
```
0 9 * * * cd /path/to/NewsTraderBot && .venv/bin/python -m scripts.collect_news >> logs/collect_news.log 2>&1
```
...
| `scripts/collect_news.py` | Собирает RSS, фильтрует релевантное, анализирует через LLM, сохраняет в БД |
| `scripts/calibrate.py` | Генерирует стратегии по всем бумагам и анализирует распределение скоринга (для настройки порогов) |
| `scripts/backtest.py` | Оценивает вердикты против фактического движения цены (требует накопленной истории новостей) |
