...
| `scripts/update_oi.py` | Синхронизирует открытые позиции фьючерсов из MOEX ISS (история `iss/history/.../forts/securities/{SECID}.json`); `--ticker <SECID>` — код фьючерса (например W4V6, можно несколько раз), `--days N` / `--from YYYY-MM-DD` — окно. Автоматически создаёт бумагу-фьючерс, если её нет в справочнике |
